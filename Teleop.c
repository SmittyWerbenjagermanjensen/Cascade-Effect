...
const int top = 9001; // the encoder reading at the top of the elevator
const int bottom = 0; // the encoder reading at the bottom
...
task main() {
  ...
  while (true) { //Literally just controls the Left Drive
	  ...
	  if (abs(joystick.joy1_y2) > 15) {
		...
		if (joy1Btn(7) && nMotorEncoder[elevator] < top) { // if 7 is pressed and the elevator is not at the top
			motor[elevator] = 75;
		}
		else if (joy1Btn(8) && nMotorEncoder[elevator] > bottom) { // if 8 is pressed and the elevator is not at the bottom
			motor[elevator] = -50;
		}
		else {
